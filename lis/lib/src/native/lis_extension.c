...
void LIS_MatrixSubtract(Dart_NativeArguments arguments) {
  LIS_INT err;
  LIS_MATRIX A, B, C;
  ...
  err = lis_matrix_subtract(A, B, C); CHKERR(err);
...
Dart_NativeFunction ResolveName(Dart_Handle name, int argc,
  ...
  if (strcmp("LIS_MatrixSubtract", cname) == 0) result = LIS_MatrixSubtract;
