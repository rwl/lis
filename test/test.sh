...
$MPIRUN $srcdir/test1.js $srcdir/testmat.mtx 0 /dev/null /dev/null

echo 'checking eigensolvers...'
$MPIRUN $srcdir/etest1.js $srcdir/testmat.mtx /dev/null /dev/null

if test "$enable_fortran" = "yes" || test "$enable_f90" = "yes"; then
  echo 'checking Fortran interface...'
  $MPIRUN $srcdir/test4f.js
  echo ' '
fi

if test "$enable_quad" = "yes"; then
  echo 'checking double-double precision operations...'
  $MPIRUN $srcdir/test5.js 200 2.0 -f double
  $MPIRUN $srcdir/test5.js 200 2.0 -f quad
fi

if test "$enable_saamg" = "yes"; then
  echo 'checking SAAMG preconditioner...'
  $MPIRUN $srcdir/test2.js 10 10 1 /dev/null /dev/null -i cg -p saamg
fi
